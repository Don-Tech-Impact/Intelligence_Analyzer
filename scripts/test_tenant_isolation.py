...
def test_database_rls():
    ...
    try:
        ...
        session.execute(text("INSERT INTO logs (tenant_id, message, timestamp, severity) VALUES ('tenant_a', 'Private Log A', NOW(), 'high')"))
        session.execute(text("INSERT INTO logs (tenant_id, message, timestamp, severity) VALUES ('tenant_b', 'Private Log B', NOW(), 'low')"))
        session.commit()
        print(f"   {GREEN}[INFO]{RESET} Sample logs inserted for 'tenant_a' and 'tenant_b'")
...
def test_api_isolation():
    ...
    print(f"    - Superadmin result for 'default': {res_default.status_code}, count: {len(res_default.json().get('data', []))}")

    print(f"    - Superadmin result for 'non_existent': {requests.get(f'{BASE_URL}/api/v1/logs?tenant_id=non_existent', headers=headers).status_code}")
