...
def setup_threat_intel():
    ...
    try:
        ...
        session.execute(text(f"""
            INSERT INTO threat_intelligence (indicator_type, indicator_value, threat_type, confidence, source, description) 
            VALUES ('ip', '{MALICIOUS_IP}', 'Botnet C2', 0.95, 'AbuseIPDB', 'Known malicious botnet node')
        """))
        session.commit()
