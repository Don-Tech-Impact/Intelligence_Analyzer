...
class AnalyticsService:
    ...
    def get_dashboard_summary(tenant_id: str, db: Session) -> Dict[str, Any]:
        ...
        if current_events == 0:
            current_events = (
                db.query(func.count(NormalizedLog.id))
                .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.timestamp >= (now - timedelta(days=14)))
                .scalar()
                or 0
            )
        ...
        asset_types = (
            db.query(
                ...
                func.count(
                    func.distinct(func.coalesce(NormalizedLog.device_id, cast(NormalizedLog.source_ip, String)))
                ),
            )
            .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.timestamp >= (now - timedelta(days=14)))
        ...
        risk_factors = (
            ...
            .filter(Alert.tenant_id == tenant_id, Alert.status == "open", Alert.severity.in_(["critical", "high"]))
            .distinct()
            .limit(3)
            .all()
        )
        ...
        return {
            "total_events": {"count": current_events, "trend": round(event_trend, 1)},
            "active_threats": {
                "count": total_threats,
            ...
            "affected_assets": {"count": affected_assets, "types": {v or "unknown": c for v, c in asset_types}},
            "risk_score": {
                "score": risk_score,
    ...
    @staticmethod
    def get_timeline(tenant_id: str, db: Session, time_range: str = "24h", bucket: str = "hour") -> Dict[str, Any]:
        """
        ...
        # Events per bucket
        events_query = (
            db.query(time_bucket.label("timestamp"), func.count(NormalizedLog.id).label("events"))
            .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.timestamp >= start_time)
            .group_by("timestamp")
            .order_by("timestamp")
            .all()
        )
        ...
        threats_query = (
            db.query(time_bucket.label("timestamp"), func.count(NormalizedLog.id).label("threats"))
            .filter(
                NormalizedLog.tenant_id == tenant_id,
        ...
        for ts in events_map.keys():
            series.append({"timestamp": ts, "events": events_map.get(ts, 0), "threats": threats_map.get(ts, 0)})

        return {"range": time_range, "bucket": bucket, "series": series}
    ...
    def get_geo_distribution(tenant_id: str, db: Session) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = (
                ...
                .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.business_context.isnot(None))
        ...
        threat_query = (
            db.query(country.label("country_name"), func.count(NormalizedLog.id).label("threat_count"))
            .filter(
                NormalizedLog.tenant_id == tenant_id,
                NormalizedLog.severity.in_(["critical", "high"]),
    ...
    def get_traffic_analysis(tenant_id: str, db: Session) -> List[Dict[str, Any]]:
        ...
        results = (
            db.query(NormalizedLog.protocol, func.count(NormalizedLog.id).label("connection_count"))
            .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.protocol.isnot(None))
            .group_by(NormalizedLog.protocol)
    ...
    def get_business_insights(tenant_id: str, db: Session, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        logs = (
            db.query(NormalizedLog.timestamp, NormalizedLog.vendor)
