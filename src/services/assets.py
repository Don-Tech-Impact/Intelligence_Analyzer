...
class AssetService:
    ...
    @staticmethod
    def get_assets(
        tenant_id: str, db: Session, page: int = 1, limit: int = 20, search: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        query = (
            ...
            .filter(NormalizedLog.tenant_id == tenant_id, uid_col.isnot(None))
            .group_by(uid_col, NormalizedLog.vendor)
        )

        # Apply search filter
        if search:
            query = query.filter(uid_col.ilike(f"%{search}%"))
        ...
        threat_counts = (
            db.query(uid_col.label("asset_uid"), func.count(NormalizedLog.id).label("threat_count"))
            .filter(
    ...
    def update_heartbeats(tenant_id: str, device_ids: List[str], db: Session):
        ...
        try:
            ...
            db.query(ManagedDevice).filter(
                ...
                or_(ManagedDevice.device_id.in_(device_ids), ManagedDevice.ip_address.in_(device_ids)),
            ).update({ManagedDevice.last_log_at: datetime.utcnow()}, synchronize_session=False)
    ...
    @staticmethod
    def get_asset_details(tenant_id: str, device_id: str, db: Session) -> Optional[Dict[str, Any]]:
        ...
        asset_info = (
            ...
            .filter(NormalizedLog.tenant_id == tenant_id, NormalizedLog.device_id == device_id)
            .group_by(NormalizedLog.device_id, NormalizedLog.vendor)
            .first()
        )
        ...
        recent_alerts = (
            db.query(Alert)
            .filter(Alert.tenant_id == tenant_id, Alert.details["device_id"].astext == device_id)
        ...
        return {
            ...
            "recent_alerts": [
                {"id": a.id, "type": a.alert_type, "severity": a.severity, "created_at": a.created_at.isoformat()}
