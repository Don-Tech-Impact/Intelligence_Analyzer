...
class ReportGenerator:
    ...
    def generate_daily_report(self, date: Optional[datetime] = None, tenant_id: str = "default") -> Optional[Report]:
    ...
    def generate_report(
        self, start_date: datetime, end_date: datetime, report_type: str = "custom", tenant_id: str = "default"
    ) -> Optional[Report]:
        ...
        try:
            ...
            # Collect report data
            report_data = self._collect_report_data(start_date, end_date, tenant_id)

            # Generate report files
            report_files = self._generate_report_files(report_data, start_date, end_date, report_type)
    ...
    def _collect_report_data(self, start_date: datetime, end_date: datetime, tenant_id: str) -> Dict[str, Any]:
    ...
    def _generate_report_files(
        self, report_data: Dict[str, Any], start_date: datetime, end_date: datetime, report_type: str
    ) -> Dict[str, str]:
    ...
    def _generate_html_report(self, data: Dict[str, Any], start_date: datetime, end_date: datetime) -> str:
...
"""
        ...
        if not top_ips:
            html += (
                "<tr><td colspan='2' style='text-align:center;'>Minimal anomalous source activity detected.</td></tr>"
            )
        else:
...
"""
    ...
    def _generate_csv_report(self, data: Dict[str, Any], output_file: Path):
        """Generate CSV report.
        ...
        with open(output_file, "w") as f:
            ...
            f.write("\n\nAlerts by Severity\n")
            df_severity = pd.DataFrame(list(data.get("alerts_by_severity", {}).items()), columns=["Severity", "Count"])
            df_severity.to_csv(f, index=False)

            f.write("\n\nAlerts by Type\n")
            df_types = pd.DataFrame(list(data.get("alerts_by_type", {}).items()), columns=["Alert Type", "Count"])
            df_types.to_csv(f, index=False)

            f.write("\n\nTop Source IPs\n")
            df_ips = pd.DataFrame(list(data.get("top_source_ips", {}).items()), columns=["IP Address", "Alert Count"])
