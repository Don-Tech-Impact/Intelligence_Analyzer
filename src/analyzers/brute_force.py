...
class BruteForceAnalyzer(BaseAnalyzer):
    ...
    def __init__(self):
        ...
        self.time_window = timedelta(seconds=config.brute_force_time_window)
    ...
    #     if not self._is_auth_failure(log) or not log.source_ip:
    #         return None
    ...
    def analyze(self, log: Log) -> Optional[Alert]:
        ...
        # Add current attempt timestamp to cache
        self._attempt_cache[source_ip].append(now)
        
        # Remove old timestamps from cache
        time_threshold = now - self.time_window
        self._attempt_cache[source_ip] = [
            ts for ts in self._attempt_cache[source_ip] if ts >= time_threshold
        ]
        
        # Check if threshold is met
        failed_attempts = len(self._attempt_cache[source_ip])
        
        # Log the current attempt count for debugging
        logger.debug(f"IP: {source_ip}, Failed attempts in window: {failed_attempts}")
        ...
        if failed_attempts > self.threshold:
            ...
            description = (
                f"Brute force attack detected from {source_ip}. "
                f"{failed_attempts} failed authentication attempts in "
                f"{self.time_window.total_seconds()} seconds."
            )
            
            # Clear the cache for this IP to avoid repeated alerts for the same event stream
            self._attempt_cache.pop(source_ip, None)
