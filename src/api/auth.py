    ...
    try:
        ...
        try:
            payload: Dict[str, Any] = jwt.decode(
                token, secret_key, algorithms=["HS256"], options={"verify_aud": False, "leeway": 60}
            )
            return payload
...
async def verify_superadmin(payload: Dict[str, Any] = Depends(verify_jwt)):
    ...
    if not (role == "superadmin" or is_admin is True):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions. Superadmin access required."
        )
    return payload
