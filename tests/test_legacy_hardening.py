...
def test_legacy_endpoint_requires_auth(endpoint):
    ...
    # verify_admin_key raises HTTPException(401 or 403) or FastAPI
    # returns 422 if header missing (if required as parameter)
    # Our implementation uses Depends(verify_admin_key) which looks at headers.
    assert response.status_code in [401, 403, 422]


@pytest.mark.parametrize(
    "endpoint", ["/stats", "/alerts", "/logs", "/analytics/protocols", "/config", "/api/dashboard-summary"]
)
...
def test_config_post_accepts_correct_key():
    ...
    response = client.post("/config", json={"brute_force_threshold": 10}, headers={"X-Admin-Key": ADMIN_KEY})
